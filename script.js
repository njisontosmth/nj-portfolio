...
function openProject(id){const p=projects.find(item=>item.id===id);document.querySelector("#dialogContent").innerHTML=`<div class="dialog-art" style="--accent:${p.color}">${p.symbol}</div><span class="dialog-status">${p.status} · ${p.progress}%</span><h2 class="dialog-title">${p.title}</h2><p class="dialog-description">${p.summary}</p><div class="dialog-columns"><div><h4>On the desk now</h4><p>${p.now}</p></div><div><h4>What it proves</h4><p>${p.learned}</p></div><div><h4>Tools & methods</h4><p>${p.tools}</p></div><div><h4>Progress note</h4><p>The percentage is a snapshot, not a performance score. It changes as the scope becomes clearer.</p></div></div>`;projectDialog.showModal()}
